...
export function ProductCard({ product, isLoggedIn = false }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!isLoggedIn && !product.showPrice) return;
    addItem({
  ...
  const priceLabel = () => {
    if (product.showPrice && isLoggedIn) return formatPrice(Number(product.price));
    if (product.showPrice && !isLoggedIn) return "Login for Price";
    return "Login for Price";
  };

  return (
    <Link href={`/products/${product.slug}`} className="group block bg-white rounded-sm overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
